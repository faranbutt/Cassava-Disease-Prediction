...
class TritonPythonModel:
    def initialize(self, args):
        self.transform_384 = Compose(
            [Resize(384, 384), Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])]
        )
        self.transform_448 = Compose(
            [Resize(448, 448), Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])]
        )
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            out1 = (
                pb_utils.get_output_tensor_by_name(resp1, "output").as_numpy().squeeze()
            )
            out2 = (
                pb_utils.get_output_tensor_by_name(resp2, "output").as_numpy().squeeze()
            )
            out3 = (
                pb_utils.get_output_tensor_by_name(resp3, "output").as_numpy().squeeze()
            )
