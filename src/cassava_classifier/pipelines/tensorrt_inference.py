...
def load_ensemble_engines(
    cfg: DictConfig, weights: Optional[List[float]] = None
) -> EnsembleTensorRTInfer:
    model_configs = [
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="TensorRT Inference for Cassava Disease Classification"
    )
    ...
    with initialize(
        version_base=None, config_path=args.config_path, job_name="trt_infer"
    ):
    ...
    if args.ensemble:
        ...
        if not engine_path.exists():
            raise FileNotFoundError(
                f"Engine not found: {engine_path}. Train model first."
            )
